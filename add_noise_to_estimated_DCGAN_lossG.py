...
for epoch in range(n_epochs):
	...
	for i, imgs in enumerate(real_eegs):
		...
		g_loss = adversarial_loss(discriminator(gen_imgs), valid) + clean_loss * aplpha
		...
		if iter % print_iter == 0:
			print ("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [C Loss: %f]" % (epoch, n_epochs, i, len(real_eegs),
															d_loss.item(), g_loss.item(), clean_loss.item()))
		print("cleaner loss", clean_loss)
	save_EEG(gen_imgs.cpu().detach().view(batch_size, 1004, 44).numpy(), 44, 200, "./generated_eegs/generated-"+ str(epoch) + "-fake-conv-add-cg")
	save_EEG(estimated.cpu().detach().view(batch_size, 1004, 44).numpy(), 44, 200, "./generated_eegs/generated-"+ str(epoch) + "-estimated-conv-add-cg")
	save_EEG(cleaned_noisy.cpu().detach().view(batch_size, 1004, 44).numpy(), 44, 200, "./generated_eegs/generated-"+ str(epoch) + "-cleaned-conv-add-cg")
