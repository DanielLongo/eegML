...
def edf_block_iter_generator(edf_file, nsamples, samples_per_chunk, 
                             dtype='int32'):
    """
...
def edf_block_masked_iter_generator(edf_file, nsamples, samples_per_chunk, 
                                    mask_array, dtype='int32'):
    """
    factory to produce generators for iterating through an edf file and filling
    up an array from the edf with the signal data starting at 0. You choose the
    number of @samples_per_chunk, and number of samples to do in total
...
def edf2hdf_f200(fn, outfn='', hdf_dir='', anonymize=False, targetfs=200.0, database_source='TUH'):
    """
    ...
    with edflib.EdfReader(fn) as ef:
        ...
        with eeghdf.EEGHDFWriter(outfn, 'w') as eegf:
            ...
            num_channels_masked = rec.attrs['number_channels'] # this was calcuated whe create_masked_record was run
            edfblock_itr = edf_block_masked_iter_generator(
                ef,
                nsamples0,
                100 * ef.samples_in_datarecord(0)*num_channels_masked, # samples_per_chunk roughly 100 datarecords at a time
            ...
            if targetfs:
                ...
                signals200Hz_t = sk_samplerate.resample(np.transpose(signals), resampleratio, 'sinc_best')
