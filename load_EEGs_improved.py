...
class EEGDataset(data.Dataset):
    def __init__(self, data_dir, csv_file=None, num_channels=19, num_examples=-1, batch_size=64, length=1000,
        ...
        if csv_file == None:
            self.filenames = get_filenames(data_dir, num_channels, self.length, min_length=100, max_length=999999,
                                           max_num=num_examples, delay=self.delay)
        else:
            self.filenames = load_filenames_from_csv(csv_file)
            random.shuffle(self.filenames)
            self.filenames = check_files(self.filenames, num_channels, self.length, min_length=100, max_length=999999,
                                         max_num=num_examples, delay=self.delay)
    ...
    def load_examples(self):
        ...
        examples = examples[:-(len(examples) % self.batch_size)] # ensures all batches are the same size
...
def check_files(filenames, num_channels, length, min_length=0, max_length=1e9999, max_num=-1, sample_frequency=200,
                delay=10000):
    num_files_checked = 0
    checked_filenames = []
    for file in filenames:
