...
def main(args):
    ...
    for epoch in range(1, args["epochs"] + 1):
        try:
            train_losses = train(epoch, model, train_artifacts_loader, optimizer, args["cuda"], args["log_interval"], save_path, args)
            test_losses = test_net(epoch, model, eval_artificats_loader, args["cuda"], save_path, args)
            writer.flush()
...
def train(epoch, model, train_artifacts_loader, optimizer, cuda, log_interval, save_path, args):
    model.train()
    ...
    for batch_idx, (div_spec_artifact) in enumerate(train_artifacts_loader):
        ...
        if cuda:
            div_spec_artifact = div_spec_artifact.cuda()


        optimizer.zero_grad()
        outputs = nn.DataParallel(model)(div_spec_artifact)
        # print("shape outputs", outputs[0].shape)
        loss = model.loss_function(div_spec_artifact, *outputs)
        loss.backward()
        optimizer.step()
        ...
        if batch_idx % log_interval == 0:
            ...
            logging.info('Train Epoch: {epoch} [{batch:5d}/{total_batch} ({percent:2d}%)]   time:'
                         ' {time:3.2f}   {loss}'
                         .format(epoch=epoch, batch=batch_idx * len(div_spec_artifact), total_batch=len(train_artifacts_loader) * len(div_spec_artifact),
        ...
        if batch_idx == (len(train_artifacts_loader) - 2):
            # print("shape outputs", outputs[0].shape)
            save_reconstructed_images(div_spec_artifact, div_spec_artifact, epoch, outputs[0], save_path, 'reconstruction_train', cur_iteration=cur_iteration)

        if args["dataset"] == 'imagenet' and batch_idx * len(div_spec_artifact) > 25000:
            print("BREAKING" * 20)
            break
...
def test_net(epoch, model, eval_artificats_loader, cuda, save_path, args):
    model.eval()
    loss_dict = model.latest_losses()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (div_spec_artifact) in enumerate(eval_artificats_loader):
            ...
            outputs = nn.DataParallel(model)(div_spec_artifact)
            latest_losses = model.latest_losses()
            model.loss_function(div_spec_artifact, *outputs)
            latest_losses = model.latest_losses()
            ...
            if batch_idx == 0:
                save_reconstructed_images(div_spec_artifact, div_spec_artifact, epoch, outputs[0], save_path, 'reconstruction_test', cur_iteration=cur_iteration)

            if args["dataset"] == 'imagenet' and batch_idx * len(div_spec_artifact) > 1000:
                break
    ...
    for key in losses:
        if args["dataset"] != 'imagenet':
            ...
            losses[key] /= (batch_idx * len(div_spec_artifact))
    loss_string = ' '.join(['{}: {:.6f}'.format(k, v) for k, v in losses.items()])
