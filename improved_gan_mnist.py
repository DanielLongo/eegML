...
def train_gan(discriminator, generator, image_loader, num_epochs, batch_size, g_lr, d_lr, dtype, filename_prefix="DCGAN-", save_images=True):
	...
	iters = 0
	BCE = nn.BCELoss()
	real_labels = autograd.Variable(torch.ones(batch_size).type(dtype))
	fake_labels = autograd.Variable(torch.zeros(batch_size).type(dtype))
...
#
			...
			# d_fake = discriminator(fake.detach()) #fake images are separated from the graph #results will never gradient(be updated), so G will not be updated
		...
		###########################
			...
			d_fake_ = discriminator(fake)
			loss_G = BCE(d_fake_, real_labels)
			loss_G.backward()
